...
These rules keep a multi-run investigation correct across interruption, context loss, agent replacement, and concurrent runs.

**One writer at a time.** `STATE.md` has a single owner. Record the session owner and checkpoint time before dependent work, and release it when finishing. The marker is advisory, not a lock: a successor finding an owner entry older than the project's stated staleness window, with no evidence of a live session, reclaims ownership and records the takeover. Never edit canonical state from two concurrent sessions; as noted above, a stale `active` status is not proof that a worker is still running.

**Record intent before actions whose completion cannot be re-derived.** Before an external, irreversible, costly, or long-running action — a paid query or purchase, an external communication or submission, a destructive or rate-limited operation, a long analysis run, or a real-world experiment — first record in `STATE.md` the exact operation, its expected observable effect, how a successor can tell whether it completed, and the verification or rollback step. Save that entry before acting. Routine reversible reading and local analysis need no such entry.

**Treat an unresolved in-flight entry as an UNKNOWN outcome.** A successor resolves it before dependent work: check for the expected artifact, output, or external effect and record what was actually found. Never assume the action succeeded, and never blindly repeat a non-idempotent or costly operation before that check.

**Order writes so an interruption is detectable.** Saving records and artifacts before updating state, as described above, means an interruption leaves unreferenced evidence that reconciliation can find, rather than a checkpoint citing evidence that was never written. Clear the in-flight entry last, after state references the saved result.

**Detect a stalled loop.** Track attempts against the current uncertainty in `STATE.md` and reset the count when the question changes. Repeating a query or analysis that produced no new information is not progress, and neither is accumulating sources that do not discriminate between live hypotheses. When consecutive attempts yield no new evidence, escalate through: change the method, source class, or discriminating test; decompose or reframe the question; work an independent uncertainty; record the avenue as ruled out; ask the smallest specific question. Never reattempt one avenue indefinitely, within a run or across runs.

**Keep negative knowledge cheap to find.** Maintain the ruled-out list in `STATE.md`: the rejected hypothesis or exhausted avenue, a link to the `D` or `E` record that rejected it, and the condition that would justify revisiting. Consult it when choosing an action, and add to it when an avenue fails for an understood reason. Never delete an entry; mark it superseded when evidence changes. This list is what lets a fresh agent skip the full record history without repeating dead ends.
